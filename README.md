...
A backend worker that runs after the user confirms. It creates a pending reservation in PostgreSQL, then **pauses the graph** (`interrupt()`) until an admin approves or rejects via the Admin UI. On approval it calls the **MCP Reservation Logger** to persist the confirmed booking to a file. The result is returned to Agent 1 as a structured `BookingResult`, which composes the final user-facing reply.

```
User confirms → Agent 1 → Agent 2 → creates pending reservation
                                   → graph pauses (interrupt)
                                   ← admin approves / rejects
                                   → (approved) calls MCP server → writes to file
                                   → Agent 2 returns BookingResult
               Agent 1 ← reads BookingResult → replies to user
```

### MCP Reservation Logger
A standalone **FastMCP** service (SSE transport, port 8520) that exposes a single tool — `write_confirmed_reservation`. Agent 2 calls this tool after every admin approval. The tool validates a shared API key and all input fields, then appends one line to `data/reservations/confirmed_reservations.txt` on the host:

```
John Doe | ABC-1234 | 2026-04-16 10:00 – 2026-04-17 10:00 | Approved: 2026-04-16 14:32:07
```
...
├── docker-compose.yml           # All services: Weaviate + PostgreSQL + chatbot + admin_api + mcp_server
├── Makefile                     # Convenience commands (optional)
├── requirements.txt             # Dev / local dependencies (testing, evaluation)
├── .env.template
├── data/
│   ├── static/
│   │   └── graph.png            # LangGraph diagram (generated by make graph)
│   ├── seeds/
│   │   └── init_db.sql          # PostgreSQL schema + seed data (150 spaces)
│   └── reservations/
│       └── confirmed_reservations.txt  # Written by MCP server on each approval (bind-mount)
├── admin_api/
│   ├── Dockerfile
...
├── mcp_server/                  # MCP Reservation Logger service
│   ├── Dockerfile
│   ├── requirements.txt         # fastmcp, uvicorn
│   └── server.py                # FastMCP server — write_confirmed_reservation tool
└── chatbot/                     # Self-contained Docker build context
    ├── Dockerfile
    ├── entrypoint.sh            # Ingest data → start Streamlit
    ├── main.py                  # CLI entry point
    ├── requirements.txt         # Chatbot service dependencies
    ...
    │   │   ├── booking_agent.py # Agent 2 — HITL booking worker + MCP log call
...
| `MCP_API_KEY` | `default-mcp-api-key` | Shared secret between the chatbot and MCP server — set to any long random string |
...
| `ADMIN_API_HOST` | `admin_api` | Admin API service name (Docker internal) |
| `ADMIN_API_PORT` | `8000` | Port for the Admin API / Admin UI |
| `ADMIN_USERNAME` | `admin` | Admin UI login username |
| `ADMIN_PASSWORD` | `admin` | Admin UI login password |
| `MCP_SERVER_HOST` | `mcp_server` | MCP server service name (Docker internal) |
| `MCP_SERVER_PORT` | `8520` | MCP server port |
| `MCP_API_KEY` | `default-mcp-api-key` | Shared secret between chatbot and MCP server |
